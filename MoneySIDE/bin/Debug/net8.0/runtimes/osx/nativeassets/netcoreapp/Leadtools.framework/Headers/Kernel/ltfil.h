...
#define FILE_ODT                          407   // OpenDocument Text file format (ODT)
#define FILE_ODS                          408   // OpenDocument Spreadsheet file format
#define FILE_ODP                          409   // OpenDocument Presentation file format
...
typedef enum
{
   // Default mode, use specified size without any resizing
...
typedef enum _L_HTMLENGINE
{
   ...
   L_HTMLENGINE_IE,
   L_HTMLENGINE_WEBKIT,
   L_HTMLENGINE_WEBVIEW2,  // uses Edge Chromium
}
L_HTMLENGINE;
...
typedef enum _L_TEXT_ENCODING
{
   ...
   L_TEXT_ENCODING_UTF32_LE,
   L_TEXT_ENCODING_UTF32_BE,
} L_TEXT_ENCODING;
...
#define L_TEXT_FLAGS_VARIABLE_WIDTH    0x0001   // If set, the output width is calculated from the longest line in the file. In this case, FILETXTOPTIONS.bWrap and RASTERIZEDOCOPTIONS.dPageWidth are ignored.
                                                // If not set, the page width is indicated by RASTERIZEDOCOPTIONS.dPageWidth
#define L_TEXT_FLAGS_VARIABLE_HEIGHT   0x0002   // If set, the output height is calculated so the whole text file will be loaded as one page. In this case, RASTERIZEDOCOPTIONS.dPageHeight is ignored.
                                                // If not set, the page height is indicated by RASTERIZEDOCOPTIONS.dPageHeight
...
typedef struct _FILETXTOPTIONSA
{
   ...
   L_UINT uFlags;                // One of the L_TEXT_FLAGS_XXX flags
   L_DOUBLE dMinPageWidth;       // if >0, minimum page width, using units from RASTERIZEDOCOPTIONS.uUnit. If 0, there is no minimum page width
   L_DOUBLE dMinPageHeight;      // if >0, minimum page height, using units from RASTERIZEDOCOPTIONS.uUnit. If 0, there is no minimum page height
#endif // #if defined(LEADTOOLS_V22_OR_LATER)
}
